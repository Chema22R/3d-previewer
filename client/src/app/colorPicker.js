...
(function(win, doc) {
    ...
    (function($) {
        ...
        function offset(el) {
            if (el === win) {
                var top = win.pageYOffset || h[scroll_top],
                right = win.pageXOffset || h[scroll_right];
            } else {
                var top = el[offset_top],
                right = el[offset_right];
        ...
        function create(first, bucket) {
            ...
            if (first) {
                picker.style.right = picker.style.top = '-9999px';
                function click(e) {
                    var t = e.target,
            ...
            $.exit = function(e) {
                ...
                if (isMobile.any()) {
                    off(on_down_mobile, H, down_H);
                    off(on_down_mobile, SV, down_SV);
                    off(on_move_mobile, doc, move);
                    off(on_up_mobile, doc, stop);
                } else {
                ...
                off(on_orientationchange, win, fitDelayed);
            ...
            if (!first) {
                if (isMobile.any()) {
                    on(on_down_mobile, H, down_H);
                    on(on_down_mobile, SV, down_SV);
                    on(on_move_mobile, doc, move);
                    on(on_up_mobile, doc, stop);
                } else {
                    on(on_down, H, down_H);
                ...
                on(on_orientationchange, win, fitDelayed);
            }
        } create(1);
        ...
        $.fit = function(o) {
            ...
            picker.style.right = '65px';  // picker.style.right = right + 'px';
            picker.style.top = top - 35 + 'px'; // picker.style.top = top + 'px';
            return trigger("fit", [$]), $;
        };
